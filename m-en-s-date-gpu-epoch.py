...
def train_evaluate_model(model, X_train, y_train, X_test, y_test, model_name, is_nn=False, use_gpu=False):
    ...
    if is_nn:
        ...
        print(f"\nTraining {model_name} neural network on {device} for {epochs} epochs:")
        print("-" * 80)
        print(f"{'Epoch':>5} {'Batch':>8} {'Loss':>10} {'Accuracy':>10} {'Progress':>10}")
        print("-" * 80)
        ...
        for epoch in range(epochs):
            ...
            # Track per-batch metrics for detailed reporting
            for batch_idx, (inputs, labels) in enumerate(train_loader):
                ...
                # Print every 5 batches or adjust based on your preference
                if (batch_idx + 1) % 5 == 0 or batch_idx == 0 or batch_idx == len(train_loader) - 1:
                    progress = (batch_idx + 1) / len(train_loader) * 100
                    print(f"{epoch+1:>5d} {batch_idx+1:>8d}/{len(train_loader)} {batch_loss:>10.4f} {batch_accuracy:>10.4f} {progress:>9.1f}%")
            ...
            print("-" * 80)
            print(f"Epoch {epoch+1}/{epochs} completed in {epoch_time:.2f}s (avg batch: {avg_batch_time:.4f}s)")
        ...
        if use_gpu:
            try:
                ...
                if hasattr(model, 'fit'):
                    ...
                    # Check if model supports partial_fit for incremental training
                    if hasattr(model, 'partial_fit') and hasattr(model, 'classes_'):
                        # For models that support incremental training (like SGDClassifier)
                        # Simulate epoch-by-epoch training
                        num_epochs = 10  # Number of epochs to simulate
                        batch_size = len(X_train_processed) // num_epochs
                        if batch_size == 0:
                            batch_size = 1
                        
                        # Initialize the model with classes
                        classes = np.unique(y_train_processed)
                        model.classes_ = classes
                        
                        print(f"Training {model_name} with simulated epochs on GPU-preprocessed data:")
                        for epoch in range(num_epochs):
                            epoch_start = time.time()
                            # Shuffle data for each epoch
                            indices = np.random.permutation(len(X_train_processed))
                            X_shuffled = X_train_processed[indices]
                            y_shuffled = y_train_processed[indices]
                            ...
                            print(f"Epoch {epoch+1}/{num_epochs}, Training Accuracy: {train_acc:.4f}, Validation Accuracy: {val_acc:.4f}, Time: {epoch_time:.2f}s")
                    else:
                        # For models that don't support incremental training
                        print(f"Training {model_name} on GPU-preprocessed data (model doesn't support epoch-by-epoch training)")
...
def main():
    ...
    # Load data
    dataF, dataI, sdn = load_data(sample_size=3000)
